    ...
    for ch, tp, date, avg, comp, open_, los in S["t_ind_rows"]:
        ...
        if open_:
            def fb_cells(n, a, b):
                ...
                ret_day = (ret_dt.split(",")[0] if "," in ret_dt else ret_dt.split(" ")[0]) if ret_dt else dd
                return (f'<td class="num">{ret_day}</td><td style="white-space:normal">{cms}</td>'
