...
table.m table.inner thead th{background:#fff;padding:12px 10px}
table.m table.inner tbody td{padding:12px 10px;background:#fff;vertical-align:middle;border-bottom:1px solid #E0E0E0}
...
TJA = dict(
    ...
    t_tp_help="トピック名をクリックすると、そのトピックのLOダッシュボード（生徒×LOの表）を表示します。平均スコアと完了は本番と同じ（スコアのあるLOの平均、トピックを完了した生徒数）。AIフィードバック列はそのトピックのフィードバックLOの提出・確認待ち・返却済みの件数と、先生がクラスで紹介する例に選んだ★の数、そして「インサイト」— 提出物と生成された下書きを観点に沿って読み、見落とし・良い傾向・紹介候補・提出状況のうち最も優先度の高いものをLLMが選んだ一言です（「詳しく」で他のインサイトと根拠の件数・引用を開きます）。率は使いません。",
