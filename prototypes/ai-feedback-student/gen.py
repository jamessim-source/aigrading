...
def t_dash_group(S, L):
    """GroupDashboard in LO Dashboard mode, on topic 7-1: the AI Feedback overview tiles where
    production puts its Questions Solved via AI tile (dropped here, PM 20 Sep), then the student × LO
    ...
    # the production tile (Questions Solved via AI) was here and was removed (PM, 20 Sep: not required)
    cards = "".join(ov_card(*c[:6], href=c[6], L=L) for c in S["t_ov"])
    # LO column headers
    heads = ""
