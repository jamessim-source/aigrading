...
TJA = dict(
    ...
    t_set_set=[("公開と提出期間", "2026/11/06 09:00 — 2026/11/13 23:59"), ("再提出", "許可する ・ 2026/11/20 23:59 まで"),
    ...
    t_set_note="設定はLOを追加したダイアログで入力した内容です。「設定を編集」で同じダイアログが入力済みで開きます。教材・提出条件・観点は「内容」タブにあります。",
    ...
    t_tp_help="トピック名をクリックすると、そのトピックのLOダッシュボード（生徒×LOの表）を表示します。平均スコアと完了は本番と同じ（スコアのあるLOの平均、トピックを完了した生徒数）。AIフィードバック列はそのトピックのフィードバックLOの提出・確認待ち・返却済みの件数と、先生がクラスで紹介する例に選んだ★の数、そして「今週の見落とし」— 提出物と生成された下書きを観点に沿って読んだ一言です（「詳しく」で根拠の件数と引用を開きます）。率は使いません。",
    t_tp_ins={1: "外れ値の除外前後で相関係数を比べた下書きが 21/30件。第6回の狙いは届いています。", 2: "有意性の検定に進まない下書きが 8/12件 ・ 次回冒頭の候補", 3: "講義内容と自分の経験を結びつけた振り返りが 15/21件 ・ 自分で問いを立てたものは 4件"},
    t_tp_more="詳しく", t_tp_less="閉じる", t_tp_ins_lbl="今週の見落とし", t_tp_search="チャプター名・トピック名で検索", t_tp_start="開始", t_tp_due="締切",
    ...
    t_gd=dict(h="AIフィードバック ・ 今週の状況", sub="11月8日〜11月14日 ・ 対象 30人",
              ...
              b3_h="今週クラスが見落とした点", b3_lo="第7回 演習レポート ・ 提出 12件とその下書き（返却済み 3）", b3_line="有意性の検定の手順が最も多く指摘されています。次回の冒頭で取り上げる候補です。",
              ...
              b3_gen="提出物と生成されたフィードバック（下書き）から、観点に沿って生成 ・ 返却前から表示 ・ 先生のみ ・ 根拠は下の件数と引用", b3_open="この提出を開く",
    ...
    t_stu_det={"第7回 演習レポート": ("2026/11/14 09:12", "", "3", "1", "2", "", "統計処理 ・ 解釈", "", "確認する"),
               "第7週 週次リフレクション": ("2026/11/15 20:11", "2026/11/16 00:05", "2", "1", "1", "", "論理構成 ・ 生成AIリテラシー", "自分の言葉で問いを立てた", "見る"),
               "第6回 演習レポート": ("2026/11/09 13:02", "2026/11/10 17:40", "3", "1", "2", "2", "Excelスキル ・ 統計処理 ・ 解釈", "", "見る")},
    t_stu_det_lbl={"sub": "提出", "ret": "返却", "cm": "コメント", "good": "良い点", "imp": "改善点", "fixed": "再提出で修正", "crit": "観点", "attempt": "2回目"},
...
TEN = dict(
    ...
    t_set_note="These are the values entered in the Add LO dialog. Edit settings reopens that dialog prefilled. The material, the submission requirements and the criteria live on the Content tab.",
    ...
    t_tp_help="Click a topic name to open its LO Dashboard (the student × LO matrix). Average Score and Completion are production's: the average over the topic's scored LOs and the number of students who completed the topic. The AI Feedback column shows the topic's feedback LO with its submitted / waiting / returned counts, the ★ number of submissions the teacher chose to show the class, and “What the class missed” — one line read from the submissions and their draft feedback along the rubric (Details opens the counts and quotes behind it). Counts, never rates.",
    t_tp_ins={1: "21 of 30 drafts compare the correlation before and after removing the outlier — Session 6's point has landed.", 2: "8 of 12 drafts never reach the test of significance · a candidate for the next lecture's opening", 3: "15 of 21 reflections tie the lecture to their own experience · 4 ask a question of their own"},
    t_tp_more="Details", t_tp_less="Close", t_tp_ins_lbl="What the class missed", t_tp_search="Search by Chapter Name or Topic Name", t_tp_start="Start", t_tp_due="Due",
    ...
    t_gd=dict(h="AI Feedback · this week", sub="8–14 Nov · 30 students",
              ...
              b3_h="What the class missed this week", b3_lo="Session 7 exercise report · 12 submissions and their drafts (3 returned)", b3_line="The test-of-significance procedure is the most-flagged point. A candidate for the opening recap next lecture.",
              ...
              b3_gen="Generated from the submissions and their draft feedback, along the rubric · available before anything is returned · teacher only · grounded in the counts and quotes below", b3_open="Open this submission",
    ...
    t_stu_det={"Session 7 exercise report": ("14 Nov 2026, 09:12", "", "3", "1", "2", "", "Statistics · Interpretation", "", "Review"),
               "Week 7 weekly reflection": ("15 Nov 2026, 20:11", "16 Nov 2026, 00:05", "2", "1", "1", "", "Logical structure · Generative-AI literacy", "Asked a question of her own", "View"),
               "Session 6 exercise report": ("9 Nov 2026, 13:02", "10 Nov 2026, 17:40", "3", "1", "2", "2", "Excel skills · Statistics · Interpretation", "", "View")},
    t_stu_det_lbl={"sub": "Submitted", "ret": "Returned", "cm": "Comments", "good": "strengths", "imp": "to improve", "fixed": "fixed on resubmission", "crit": "Criteria", "attempt": "2nd attempt"},
...
TSCREENS = ["T-Book", "T-Dialog", "T-Material", "T-Settings", "T-Created", "T-Queue", "T-Detail", "T-List", "T-Review", "T-DashTopic", "T-DashGroup", "T-DashStudent"]
...
<div class="tscroll">
  ...
  <div style="display:flex;flex-direction:column;gap:16px">
    ...
    <div class="tpaper"><div class="ph"><h3>{S["t_s_sum"]}</h3><a class="tbtn sm" href="{tfn("T-Settings", L)}">{mi("edit", 18)}{S["t_edit_in_bm"]}</a></div><div class="pb"><dl class="tkv">{kv}</dl></div></div>
  </div>
</div>
</div>'''
...
def class_missed(S, L):
    ...
    return (f'<div class="gd"><div class="blk" style="grid-column:1 / -1"><div style="display:flex;align-items:center;justify-content:space-between;gap:12px"><h4>{mi("rateReview", 14)}{G["b3_h"]}</h4>'
            f'<span class="tbtn sm" style="height:26px">{mi("autorenew", 16)}{S["t_regen"]}</span></div><span class="helper" style="margin:0">{G["b3_lo"]}</span>'
            f'<p class="bline">{G["b3_insight"]}</p><div class="bquotes">{quotes}</div><div class="bchips">'
...
def t_dash_topic(S, L):
    ...
    for i, (ch, tp, avg, comp, fb) in enumerate(S["t_tp_rows"]):
        ...
        if fb:
            ...
            ins_line = f'<span class="tp-ins">{mi("rateReview", 14, "#757575")}<span><b>{S["t_tp_ins_lbl"]}:</b> {ins}</span>{more}</span>' if ins else ""
    ...
    for ch, tp, date, avg, comp, open_, los in S["t_ind_rows"]:
        ...
        if open_:
            def lo_detail(n, a, b):
                ...
                Lb = S["t_stu_det_lbl"]
                parts = [f'{Lb["sub"]} {sub_dt}' + (f' ・ {Lb["attempt"]}' if b == "resub" else "")]
                if ret_dt: parts.append(f'{Lb["ret"]} {ret_dt}')
...
ttitles = ["T1 · Book detail — the tree, Add LO", "T2 · Add Learning Objective — AI Feedback type, its settings",
           ...
           "T6 · Submission Grading — the queue, filtered to AI Feedback", "T7 · Overview — who has submitted", "T8 · The LO's submissions — same table, one LO", "T9 · Review and return — the grading layout",
           "T10 · Group Dashboard — Topic Dashboard, the AI Feedback LO per topic, what the class missed", "T11 · Group Dashboard — LO Dashboard, AI Feedback in the student × LO matrix", "T12 · Student Dashboard — one student's AI Feedback"]
for lang, S in (("ja", JA), ("en", EN)):
...
for i, key in enumerate(["t1", "t2", "t3", "t4", "t5", "t6", "t7", "t8", "t9", "t10", "t11", "t12"]):
