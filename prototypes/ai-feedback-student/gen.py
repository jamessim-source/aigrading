...
table.m table.inner th:first-child,table.m table.inner td:first-child{min-width:170px}
...
TJA = dict(
    ...
    t_ind_cols=["チャプター名", "トピック名", "学習日", "平均スコア", "完了"], t_sub_cols=["学習目標", "最終提出", "最新スコア", "最高スコア", "返却", "コメント", "観点", ""],
...
TEN = dict(
    ...
    t_ind_cols=["Chapter Name", "Topic Name", "Study Date", "Average Score", "Completion"], t_sub_cols=["Learning Objective", "Latest Submission", "Latest Score", "Highest Score", "Returned", "Comments", "Criteria", ""],
    ...
    for ch, tp, date, avg, comp, open_, los in S["t_ind_rows"]:
        ...
        if open_:
            def fb_cells(n, a, b):
                ...
                if not det:
                    return f'<td class="num">{dd}</td><td>{dd}</td><td>{dd}</td><td></td>'
                ...
                return (f'<td class="num">{ret_dt.split(" ")[0] if ret_dt else dd}</td><td style="white-space:normal">{cms}</td>'
                        f'<td style="white-space:normal;font-size:12px">{crit}</td>'
            ...
            def lo_name(n, a):
                ...
                return n + (f'<span class="hl-why" style="display:block;margin-top:2px">{hl}</span>' if hl else "")
            inner = "".join(
                f'<tr><td style="white-space:normal">{lo_name(n, a)}</td><td class="num">{d}</td><td>{sub_val(a)}</td><td>{sub_val(b)}</td>{fb_cells(n, a, b)}</tr>' for n, d, a, b in los)
...
TNOTES = {
    ...
    "t12": "StudentDashboard as production renders it: the Student List (add-student icon, name and year, the selected one marked with the blue bar), the student's name, Course / Book / Filters / Apply, and the chapter / topic table with Study Date, Average Score and Completion, expandable to the LO rows (Learning Objective / Latest Submission / Latest Score / Highest Score). AI Feedback LOs sit in those rows with their status chip where a score would be, and 再提出 1回 where production shows the highest score. WHAT IS NEW is in the LO table itself (PM, 20 Sep: merged into the existing matrix rather than a table of its own; then, PM 20 Sep, fitted as columns rather than a detail row — the submission date was already a column): four feedback columns after production's four — 返却 Returned date, コメント the count split into 良い点 / 改善点 and how many points were 修正済み fixed on resubmission (the revision trail from screen 7 seen from the teacher's side), 観点 the rubric criteria the comments touched, and 確認する / 見る into the review — with the ★ reason under the LO name where the teacher highlighted it; other LO rows show -- there. A separate paper below the table — count tiles, a submissions table and a by-criterion profile — was built first and removed in the merge. 7-2 is expanded so the weekly reflection shows too.",
}
MNOTES = {
