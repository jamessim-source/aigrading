...
**What these confirmations do to the document.** Three readiness-gate blockers close (the engine contract, the theses contradiction, the one-feature-or-two question), one closes in Section 1 (U10), and C7 and C8 shrink to the LO surface and the Kindai volumes. **What they do not do** is author Parts A and B, define the rest of C3, or produce acceptance criteria — those remain the open work in D.

**A boundary worth restating for whoever writes the AI Grading PRD.** Four things were drafted in this file and are now that PRD's to own: the engine contract (inputs, outputs, the trigger rule, ownership and versioning), the OCR-versus-clean-text asymmetry between the two scoring consumers, [S3]'s setup phasing, and the Correspondence Division's 62,000-report volume with its corpus re-grading question. They are in this file's v1.1 history rather than deleted.
...
5. James Sim: fill the rest of C3 — the attention-audit row, the appeal row, concurrency, per-student due dates, and the failure paths. The DEFINED half is done; these are the policy and cross-cutting ones that cascade.
