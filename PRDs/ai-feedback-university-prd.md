...
| P7 | 24 Sep | **The learner flow, PC and mobile** (Koki's prototype adopted, trimmed to the C10 rules): the practice LO screen — StatBox 問題数 / 正解数 with rounds of 10, selectable set rows with 完了 / 未完了, a print icon on complete sets, the ＋ row, and a CTA that follows the selected set; **crop** the linked PDF with the existing Mana AI frames relabelled 演習をつくる → (up to five, may span pages); **切り取った範囲** — one row per detected question, the read-only destination line, one slider per question capped by the shallowest remaining stock (the maximum is stated, the stock never is), the live total, and NO_MATCH (*Please ensure your crop contains the question in full and try again* / 問題全体が枠に入るように切り取って、もう一度お試しください) surfacing here before any count is chosen; the **wait state** for creation only (retrieval, not generation); **practice** as the AI Tutor web app's MCQ module in a webview — one attempt per question, explanation, 次へ, round end; **print** with question and options only | P4–P9, P10–P14 | C9.1 — the mobile flow reuses the study-guide crop the AI Feedback snap flow does not; no AI Feedback rule changes |
