...
| P3 | 24 Sep | **The practice LO's page**: the linked LO, the prepared source questions (three; stock counts never reach students), the sets students created with a way into the dashboard, its availability (a start date from Course Management, **no end date** — a practice LO has no deadline), and what it does not have: no completion status, no mastery level, no delete, no teacher override | P3 | C4 |
