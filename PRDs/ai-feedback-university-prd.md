...
|------|---------|--------|--------|--------|-------------|
| 2026-09-22 | v1.3 | James Sim (drafted with Claude) | **C3 cleared, and submission dates move to the course.** The fourteen rows left in the business-logic table were resolved in one pass (**C12 P11**): six removed as behaviour this product does not have — low-confidence routing, low OCR confidence, an out-of-scope submission, suspected AI generation, and **both rubric-defect rows, whose bugs are fixed**; one more (two reviewers on one submission) to the AI Grading PRD; and seven defined — a rubric may be edited with every later generation using it, disabling the feature stops new generations, the product is model-agnostic, a student may replace without a cap until the due date, a late submission is refused unless the due date is extended, and a published LO before its start date shows the date and is not clickable. **The structural one: the submission start and due date move from the LO to the Course Management page**, the book links to the course, and the course's window gates submissions — which answers the per-class variation open since 17 Sep, **supersedes the LO-level dates in the prototype**, and leaves two questions open (whether the LO keeps dates, where the resubmission date lives). C3's table is done; what remains is the failure paths, admin bypass and the Env/Tenant labels | Draft | — |
...
3. **A Feedback LO in the learner app** — feedback as a distinct **LO type** inside the existing LO submission flow, carrying **its own icon** on the To-do list alongside PDF, video, quiz and AI flash-card types. **Confirmed and shaped by the PM in the prototype (18–20 Sep — C11):** the LO is created in Book Management's Add Learning Objective dialog as one more LO type, in the hierarchy Book → Chapter → Topic → LO per the [Book Management PRD](https://manabie.atlassian.net/wiki/spaces/PRDM/pages/1133903925/Book+Management) **[S21]**; its settings are resubmission (default off), accepted submission methods (file / photos / typed, 500-character limit on typed) and teacher review (default on); its content page holds the teacher's material, the generated **basic requirements** that gate submission, and the generated **rubric** that shapes the comments. **The submission window is not the LO's (PM, 22 Sep — C12 P11): start and due dates are managed on the Course Management page, the book is linked to the course, and the course's window gates submissions** — which also drives when the LO becomes clickable and when it reaches the To-do. Submissions are processed under **Course › Submission Grading**, production's existing home for work waiting on a teacher.
...
## C3. Business logic & behavior rules — *now largely defined; the gap is failure paths*

The table below is **the list of cells that must not be empty**, not a specification. **DEFINED** rows were settled by the PM in the prototype review (C11) or the scope confirmations (C12) and are behaviour the product must have.

> **What left this table on 22 Sep.** **Eight scenarios went to the AI Grading PRD (C12 P10, P11):** cold start · past submissions without scores · an existing institutional rubric · low trial-grading agreement · a student appealing a returned grade · two-stage TA-then-instructor review · an undetected attention-audit item · two reviewers open on the same submission. Each presupposes **scoring, or the calibration and marking hierarchy around it**, and this product returns comments, not marks. **Six more were removed outright (C12 P11)** because they describe behaviour this product does not have: routing a low-confidence item · low OCR transcription confidence · a submission out of the course's scope · suspected wholesale AI generation · and the two rubric-defect rows (teacher rubric destroyed, student rubric colliding with the teacher's) — **both defects are fixed**. **Do not re-add any of them without re-adding what they assume.**

| Scenario / State | Expected behavior | Notes / edge cases |
|---|---|---|
| No submission yet / first use of an assignment | **DEFINED (C11.3, C11.5)** — the LO's overview under Submission Grading shows Submitted / Waiting for review / Returned at 0 and the LO's settings read-only; the Topic Dashboard lists the LO with its submission window and 0 counts and no insight line; the student's To-do shows the LO once the window opens | The rubric and requirements are already on the LO's Content tab before any submission. Dates shown are the course's (C12 P11) |
| Submission in draft (formative) vs final | **DEFINED (C11.1)** — until the due date the student can view and **replace** the file, pages or typed text; the basic-requirement checks run on each replacement. Nothing is reviewed or returned before the due date. A further round exists only when resubmission is on for the LO | No "learning support" label: the student sees no draft at all before return |
...
| Instructor edits the draft | **DEFINED (C11.4)** — there is no score. The teacher **edits or deletes individual comments** in the report and writes the note; an edited comment carries an edited mark | What the diff feeds: undefined |
| Instructor rejects and regenerates | **DEFINED (C11.4)** — 差し戻す **Sent Back**: the submission goes back to the student for another submission (error tone); regenerating the draft is an action in the review screen's kebab | |
| Instructor flags for interview / viva | **N/A (C12 P5)** — the viva is the client's own workflow, not a feature; nothing in the product flags for it | |
| Rubric is edited after submissions have already been commented against | **DEFINED (PM, 22 Sep — C12 P11)** — **the teacher may edit the rubric at any time; every generation after the edit uses the edited rubric.** Comments already written are not re-generated and not retro-tagged | No versioning, no re-run, no disagreement report — those are the KIV feature (C12 P3). Worth saying in the UI that the change applies from the next generation on |
| Feature is disabled mid-term with submissions in flight | **DEFINED (PM, 22 Sep — C12 P11)** — **no new generations happen.** Work already submitted and already generated is unaffected | Small residual: whether a draft already generated but not yet returned can still be returned |
| Model is replaced | **DEFINED (PM, 22 Sep — C12 P11)** — **the product is model-agnostic.** There is no score to re-validate, so no re-validation ritual | [S3 ch.6]'s model-agnostic storage schema is the part that still matters technically; its trial-grading re-validation left with C12 P10 |
| Student keeps replacing the submission until the checks pass | **DEFINED (PM, 22 Sep — C12 P11)** — **no cap.** The student may keep updating until the due date; the basic-requirement checks run on each replacement | [S6]'s 36%-avoidable-resubmission figure is the argument *for* letting them retry, not against |
| **Teacher publishes a whole semester of AI Feedback items at once** | **DEFINED (C11.3; dates per C12 P11)** — Confirm creates the LO **Unpublished**; **Publish** is a separate action on the LO page or the tree row. A published LO whose start date has not passed **shows its start date on the course tab and is not clickable until that date**. Submission windows come from the **course** (see the due-date row), not from the LO | Notification on start is still undefined. A **blank** start date is not addressed: on the rule above it would leave the LO open from publication, so either make the field required or say that blank means open |
| **AI Feedback item passes its start date** | **DEFINED (C11.1, C11.3)** — appears on the To-do with the Feedback type icon, and the LO becomes clickable on the course tab | Notification and ordering against other LO types: undefined |
| **AI Feedback item passes its due date without submission** | **DEFINED (PM, 22 Sep — C12 P11)** — To-do shows it overdue in red, and **a late submission is not accepted**. The one way in is for the teacher to **extend the due date**, which is a course-level change (see below) | |
...
| **Resubmission allowed for the LO** | **DEFINED (C11.1, C11.3)** — per-LO toggle, default off. The second submission screen lists the previous comments as a checklist; a returned resubmission opens with what changed; secondary chip **Resubmitted** in the tables; 再提出 1回 sits under the submission date on the Student Dashboard, never in a score column | **Where the resubmission date lives is open** since dates moved to the course (C12 P11) |
...
| **Due date differs per student or per class** | **DEFINED (PM, 22 Sep — C12 P11): dates belong to the course, not the LO.** The **submission start and due date are managed on the Course Management page**; the book is linked to the course; **the course's submission window gates student submissions**. Two courses using the same book therefore run the same assignment on their own dates, which is what "differs per class" actually needed | This supersedes the LO-level start and due dates recorded in C11.3 and drawn in the prototype. **Two things it does not say and engineering must not infer:** whether the LO keeps dates at all (as a default the course overrides, or not at all), and where the **resubmission** date now lives. Per-*student* variation remains unsolved, and Koki's warning stands |

**Mandatory edge-case checklist:**
- [x] Empty / no-data / first-use state — **defined for the teacher's screens (C11.3, C11.5)**; on the student side a published LO before its start date shows the date and is not clickable (C12 P11)
- [x] Each status the entity can be in — **teacher-side (C11.4):** Not Reviewed · In Review · Returned · Sent Back, plus Auto-returned and Resubmitted; **student-side (C11.1, C12 P11):** not submitted · submitted (waiting) · returned · awaiting resubmission (再提出待ち) · overdue, with a late submission refused. Appeal left with the AI Grading PRD (C12 P10); **failed generation is the one state still unnamed** — see the failure-paths item below
- [ ] Manual override / admin bypass behavior
- [ ] **Error and failure paths — now the largest gap in C3.** Generation failure, model timeout, partial extraction, a corrupt or unreadable upload: none is defined, and the student-facing behaviour matters because the student is never told AI is involved
- [x] ~~Concurrency~~ — **left with the AI Grading PRD (C12 P10, P11).** Two staff opening the same draft is that PRD's row now
- [ ] **Configurable vs hardcoded — every value listed in A2 needs an explicit Env-level or Tenant-level label.** None has one.

## C4. User journeys — *five exist as the prototype [S20]; one persona and one screen still owed*
...
**Not yet written:** academic affairs (rubric governance, audit log, consistency report — nothing prototyped, and rubric versioning is KIV per C12 P3). **The TA journey is no longer owed here:** two-stage TA-then-instructor review left with the AI Grading PRD (C12 P10), so this PRD assumes one reviewer per LO. **Newly owed (C12 P11):** the **Course Management journey** — setting the submission start and due date on the course the book is linked to, and extending the due date to admit a late submission.
...
- **Learner app — what the 17 Sep AI Direction Discussion settled, and what C12 P11 has since answered.** Settled on 17 Sep: AI Feedback is a distinct **LO type** with its own To-do icon, and the To-do needs a due-date layer built on top of existing LO functions. **Where that layer lives is now decided (C12 P11): on the course.** Submission start and due dates are managed in Course Management, the book is linked to the course, and the course's window gates submissions and drives the To-do. What remains open: whether the LO keeps dates at all, where the resubmission date lives, how this interacts with study plan, and **per-student variation, which is still unsolved**. **Bunsuke owes a proposal on LO type refactoring; Koki owes a demo of start/due-date behaviour** — the demo should now be built against course-level dates.
...
- [x] ~~**Part C3 is only half defined**~~ — **largely closed (C12 P10, P11).** Eight scenarios went to the AI Grading PRD, six were removed as behaviour this product does not have, and the rest are DEFINED. **What is left is not the table but the checklist:** error and failure paths (generation failure, model timeout, partial extraction, an unreadable upload) — now the single largest gap in C3 — plus manual override / admin bypass, and the Env-versus-Tenant labels for every configurable value in A2.
- [ ] **Error and failure paths are now the largest gap in C3.** Generation failure, model timeout, partial extraction, an unreadable upload: none is defined, and the student-facing behaviour matters precisely because the student is never told AI is involved. **Failed generation is also the one entity state still unnamed.**
- [ ] **Manual override / admin bypass, and the Env-versus-Tenant label on every configurable value in A2.** Neither has been touched; the 500-character limit, the upload cap and the accepted file types are the concrete cases (C11.9 Q7).
- [ ] **The course-level dates decision (C12 P11) is not yet drawn or specified.** It supersedes the LO-level dates in the prototype, so the Add LO dialog and Settings tab are out of date, a **Course Management journey is owed in C4**, and two questions are open: whether the LO keeps dates at all, and where the resubmission date lives. **Per-student due dates remain unsolved.**
...
- [ ] **The AI Grading PRD does not exist**, and six things now depend on it: the engine contract, [S3]'s setup phasing, AI Red-Pen Grading, the Correspondence Division's grading assistance, the seven C3 scenarios of C12 P10, and the concurrency row of C12 P11. **Moving them out of this PRD only helps once that PRD is started.**
...
5. James Sim: close what is left of C3 — **the failure paths first** (generation failure, model timeout, partial extraction, an unreadable upload), then manual override / admin bypass and the Env-versus-Tenant labels. The scenario table itself is done (C12 P10, P11).
6. James Sim: specify the **course-level submission window** (C12 P11) — the Course Management screen, whether the LO keeps dates, where the resubmission date lives — and have the prototype redrawn to match.
7. Koki Misawa / Bunsuke Itamura: deliver the start/due-date demo — **now against course-level dates** — and the LO-type refactoring proposal, which C3 and C7 depend on.
8. Tech Lead: size the Kindai burst (~140 submissions within hours of a lecture), the feedback-generation latency and the insight line's per-LO cost before any commitment.
9. James Sim: ratify or replace the prototype defaults in C11.7, then write C6's acceptance criteria from C3, C11 and C12.
10. James Sim: start the **AI Grading PRD**, or name who does. Six decided-out items are homeless until it exists.
...
| **Flag default at release** | *TBC — likely off pending client go-ahead, given U1.* **What the flag does mid-term is defined (C12 P11): disabling stops new generations; work already done is untouched** |
