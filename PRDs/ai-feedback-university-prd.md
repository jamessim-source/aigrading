...
|------|---------|--------|--------|--------|-------------|
| 2026-09-23 | v1.5 | James Sim (drafted with Claude) | **Join the course by QR code.** From a prototype comment on the Course Management board: the AI Tutor class page's existing **Share Access** dialog (QR code + Download, class code + Copy) is reused for the course — a share icon on each course row and a Share Access button on the course detail, so students scan or type a code to join and see the course's LOs from their start dates. Recorded in C11.10 (W8); C4 updated | Draft | — |
