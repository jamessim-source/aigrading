...
| | |
|---|---|
| **Status** | Draft — Parts A–B not yet authored; Part C carries the prototype-review decisions of 18–21 Sep (**C11**) and the scope confirmations of 22 Sep (**C12**) |
...
**Still owed:** the **Course Management journey** — setting the submission start and due date on the course the book is linked to, and extending the due date to admit a late submission. **Not yet written:** academic affairs (rubric governance, audit log, consistency report — nothing prototyped, and rubric versioning is KIV). There is no TA journey: two-stage TA-then-instructor review belongs to the AI Grading PRD, so this PRD assumes one reviewer per LO.
...
- [x] **Designs exist** — the clickable prototype [S20] covers the five journeys in C4 (no Figma). Translations exist for every string but are **unconfirmed**.
...
5. James Sim: specify the **course-level submission window** — the Course Management screen, whether the LO keeps dates, where the resubmission date lives — and have the prototype redrawn to match.
