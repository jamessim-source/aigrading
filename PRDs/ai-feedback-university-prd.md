...
**Teacher — the submission window, in Back Office › Course › Course Management (boards T6–T8).** Course list (Course Name · Teaching Method · Course Type · Subject; two courses — a Thursday and a Friday class — carry the same statistics book) → the course's detail page on its **Books** tab (production's info alert: availability dates for each LO in a book are set per book; the book table) → the book's **Learning Objectives Availability** page: Chapter · Topic · LO (with its type icon) · **Start Date** · **End Date**, chapter and topic cells merged over their rows, **Edit Date** turning every date cell into an input with Cancel / Save, Import/Export for CSV. For an AI Feedback LO the Start Date is when it appears on the student's course tab and To-do, the End Date is the due date (submit and replace until it; the teacher reviews after it; a late submission is refused unless this date is extended here). One column is drawn as a proposal: **Resubmission Due**, shown only for LOs with resubmission on — the place the resubmission date could live now that dates left the LO. This is production's `CourseBookDetail` page, reused as it is; nothing here is new for AI Feedback beyond that column. **Students join the course by QR code**: the AI Tutor class page's existing **Share Access** dialog (`AIClassShare` — the QR code with Download, the class code with Copy, Close) opens from a share icon on each course row (T6) and a Share Access button on the course detail (T7); a student who scans it or types the code is enrolled and sees the course's LOs from their start dates. The course's **Student** tab (T7, production's `StudentTab`: Student Info with its Action menu, name search and filters, the student table — Student Name · Academic Year · Location · Enrollment Date · Class · School · Study Plan) lists who is in the course, with one proposed column, **Joined via**: QR code or code entered with the moment they joined, or added manually with who added them. **Add course** (T6, production's `DialogUpsertCourse` + `CourseForm`: a full-screen dialog — course icon, Course Name, Location, Teaching Method, Course Type, Subject, the Adaptive switch, Book — Cancel / Save) is where a course is created and its book linked; nothing in it is new for AI Feedback.
...
- **The To-do date layer.** AI Feedback is a distinct **LO type** with its own To-do icon, and the To-do needs a due-date layer built on top of existing LO functions [S8a]. **That layer lives on the course:** submission start and due dates are managed in Course Management, the book is linked to the course, and the course's window gates submissions and drives the To-do. Open: whether the LO keeps dates at all, where the resubmission date lives, how this interacts with study plan, and **per-student variation, which is still unsolved**. **Bunsuke owes a proposal on LO type refactoring; Koki owes a demo of start/due-date behaviour**, built against course-level dates.
...
| W2 | 23 Sep | **Course detail, Books tab**: production's alert that LO availability dates are set per book, and the book table linking to the book's availability page. The book is linked to the course here (Assign Books) | T7 | C4 |
...
| W8 | 23 Sep | **Join by QR code** (prototype comment on T6: *add a way for students to join the class by QR code, which is a function on the existing AI Tutor page*). The AI Tutor class list's share icon and its **Share Access** dialog (`ClassListTable` → `AIClassShare`: QR of the class code + Download, the code + Copy, Close) reused for the course — on each course row (T6) and beside the ⋮ on the course detail (T7). Scanning or typing the code enrols the student in the course; the course's LOs then appear from their start dates | T6, T7 | C4 |
| W9 | 23 Sep | **The course's Student tab** (prototype comment on T7: *add the student list here who joined the course by code or has been added to the course manually*). Production's `StudentTab` as it is — Student Info, the Action menu, name search and Academic Year / Class / School / Location filters, the table (Student Name · Academic Year · Location · Enrollment Date · Class · School · Study Plan) — with **one proposed column, Joined via**: QR code / code entered with the join time, or added manually with who added them. A self-joined student has no class or study plan until the teacher assigns them. Drawn to be decided | T7 | C4 |
| W10 | 23 Sep | **Add course** (prototype comment on T6: *add the add course ux flow based on the production code*). Production's `DialogUpsertCourse` in add mode — a full-screen dialog with `CourseForm`: course icon, Course Name (required), Location, Teaching Method (required; Individual / Group), Course Type, Subject, Adaptive switch (off → Book), Book (multiple) — Cancel / Save; Save adds the course to the list with the created-successfully snackbar. Drawn filled in with a third class of the statistics course linking the same book. Nothing new for AI Feedback | T6 | C4 |
