...
|------|---------|--------|--------|--------|-------------|
| 2026-09-24 | v1.8 | James Sim (drafted with Claude) | **AI Practice fitted into the prototype [S20], from the AIpractice repository [S24]** (PM: *incorporate the feature into your artifact prototype working seamlessly with the current book management, course management and group and student dashboards as well as the student app and web experience*). The Similar Questions Practice LO is drawn as one more LO in the Kindai statistics course — Topic 7-1, linked to the Session 7 lecture-slides PDF — so Book Management, Course Management, the three dashboards and the student PC and mobile screens all show it beside the AI Feedback LO; fourteen new boards per language (three Back Office, six student PC, five mobile) carry the learner flow. Its rules are those of the AI Practice PRD's C10 as the PM decided them on 24 Sep, taken from [S24] and **not re-decided here**; recorded in C11.11 and C4. **Parts A–C otherwise unchanged** | Draft | — |
...
**AI Practice beside AI Feedback (boards P1–P14, drawn 24 Sep).** Not this PRD's feature — the Similar Questions Practice LO has its own PRD, whose C10 rules are in [S24] — but the same teacher and the same students meet both in one course, so the prototype carries it through every surface above: the Add LO dialog's 類題演習 LO type with its required **Linked source LO** (P1), the source PDF LO's **Available as practice source** switch and **Linked by** list (P2), the practice LO's page (P3), a Start-only row on Learning Objectives Availability (T8), a practice column and insight line on the dashboards (T13–T15), and the student flow — sets → crop the PDF with the Mana AI frames → 切り取った範囲 with a per-question count → creating → practice → print — on PC (P4–P9) and mobile (P10–P14). C11.11 lists what was drawn and which of this PRD's surfaces it touches.
...
### C11.11 AI Practice fitted into the prototype (24 Sep)

Drawn from the AI Practice repository [S24] — its README, `docs/prototype-plan.md`, `docs/c10-finalized-logic.md` (that PRD's C10 as the PM decided it on 24 Sep), the `contract/openapi.yaml` + `mock-service/seed.py` mock and Koki Misawa's clickable prototype on branch `prototype` — at the PM's request that the feature work *seamlessly with the current book management, course management and group and student dashboards as well as the student app and web experience*. The practice feature's rules are **not** this PRD's to decide and none is changed here; what this section records is how the two LO types sit together on the surfaces this PRD specifies.

| # | Date | What was drawn | Where | PRD effect |
|---|---|---|---|---|
| P1 | 24 Sep | **The practice LO in the book tree.** Topic 7-1 gains 第7回 類題演習（相関分析）— a plain Learning Objective with `ai_practice = true` — beside the AI Feedback LO; the Session 7 lecture-slides PDF LO it links to is marked 演習の元 (source). The Add LO type menu lists 類題演習 LO as NEW; the dialog's General Info is the standard one, its Settings block is the **required Linked source LO** picker fed by the book's eligible LOs (production's empty-state copy: *No eligible LOs in this book. Turn on "Available as practice source" on a PDF LO first*), a note of the fields this type hides (max score, pass score, manual grading, AI Tutor, password) and the gate (`Syllabus_BackOffice_AIPractice` flag **and** `syllabus.ai_practice.is_enabled` tenant setting) | T1, T2, P1 | C4; C5 (one more LO type in the same dialog as AI Feedback) |
| P2 | 24 Sep | **The source LO's Settings tab**: production's read-only settings, then **Available as practice source** — shown only for supported types (v1: a PDF LO), default off, locked with the reason once students have sets — and **Linked by**, the practice LOs pointing here. One eligible LO may be linked by several practice LOs | P2 | C4 |
| P3 | 24 Sep | **The practice LO's page**: Settings shows **only the linked source LO** (PM comment on P3: *Show only linked source LO*), plus a dashboard button; a "Visible to students as" paper was drawn and removed the same day (PM: *remove this section*). Drawn first and then taken off the page, now recorded in the board's note: the prepared source questions (three; stock counts never reach students), the sets students created, its availability (a start date from Course Management, **no end date** — a practice LO has no deadline), and what it does not have: no completion status, no mastery level, no delete, no teacher override | P3 | C4 |
| P4 | 24 Sep | **Course Management**: the practice LO row on Learning Objectives Availability carries a Start Date and an empty End Date, under the same study-plan-backed dates as every other LO (W11) | T8 | C3, C7 — one more LO type whose window is a study-plan item |
| P5 | 24 Sep | **Dashboards.** Topic mode: one insight line under 7-1 (sets created ・ students ・ accuracy) linking to LO mode. LO mode: a practice column whose header reads sets · students · done / correct; a cell shows the student's set count, done / total ・ correct, an accuracy bar, and — separately — an informal line for ad hoc widget sessions, which never count as sets. Student Dashboard: the practice row shows 途中 with its sets line, no score (the column reads `--`), and a link into the group view | T13, T14, T15 | C10.0 — the AI Feedback column and the practice column coexist in the same matrix; Score + Status stays the AI Feedback rule |
| P6 | 24 Sep | **Student LO list and To-do**, PC and mobile: the ✦ practice card with its set count and no completion chip, beside the AI Feedback LO's status chips. The practice LO is in To-do because it is open, not because anything is due | Screen 1, M1, 05-Todo | C3 (To-do shows open LOs of both kinds) |
| P7 | 24 Sep | **The learner flow, PC and mobile** (Koki's prototype adopted, trimmed to the C10 rules): the practice LO screen — StatBox 問題数 / 正解数 with rounds of 10, selectable set rows with 完了 / 未完了 and a print icon on every set (PM comment on P4, 24 Sep: *print pdf should be available at any point of time for a generated set* — so an unfinished set prints too, and the CTA for an unfinished set carries print beside resume), the ＋ row, and a CTA that follows the selected set; **crop** the linked PDF with the existing Mana AI frames relabelled 演習をつくる → (up to five, may span pages); **切り取った範囲** — one row per detected question, the read-only destination line, one slider per question capped by the shallowest remaining stock (the maximum is stated, the stock never is), the live total, and NO_MATCH (*Please ensure your crop contains the question in full and try again* / 問題全体が枠に入るように切り取って、もう一度お試しください) surfacing here before any count is chosen; the **wait state** for creation only (retrieval, not generation); **practice** as the AI Tutor web app's MCQ module in a webview — one attempt per question, explanation, 次へ, round end; **print** with question and options only | P4–P9, P10–P14 | C9.1 — the mobile flow reuses the study-guide crop the AI Feedback snap flow does not; no AI Feedback rule changes |

Open for the AI Practice PRD, not this one: whether a practice LO should ever appear in Submission Grading (it has nothing to review — the prototype keeps it out of T9–T12), and whether the Topic-mode insight line belongs to the LLM-ranked insight of C10.0 or is a plain count.
...
| S24 | `jamessim-source/AIpractice` — `README.md`, `docs/prototype-plan.md`, `docs/c10-finalized-logic.md` (the AI Practice PRD's C10 as decided by the PM, 24 Sep), `contract/openapi.yaml`, `mock-service/seed.py`; branch `prototype`: `prototype/sqp-prototype.html` (Koki Misawa's clickable learner flow) | Repository — the Similar Questions Practice LO's rules and its learner flow, fitted into the prototype [S20] as C11.11 | 24 Sep 2026 |
